...
{	
	...
	m_MapImg[2].Create("./resource/Img/map1/BG_Arkanoid/back2.png", false, D3DCOLOR_XRGB(0, 0, 0));
}
...
void Map::Draw()	//스테이지별로 레이어 나눠놓기(이미지 위치)
{
	if (m_Stage == 1)
	{
		...
		dv_font.DrawString("SCORE", 610, 25);   //글자출력
	}
	else if (m_Stage == 2)
	{
		m_MapImg[2].Render(415, 105, 0, 1.05, 1);	//맨뒤에 숫자 2개는 크기 1,1 은 원본크기
		m_MapImg[1].Render(397, 100, 0, 1.05, 1);
		dv_font.DrawString("SCORE", 610, 25);   //글자출력
	}
}
