	...
	if (Gmanager.m_GameStart == true)
	{
		for (int i = 0; i < 5; i++)	//2차원배열 [5],[13]이라 2번 for문 돌리기
		{
			for (int j = 0; j < 13; j++)
			{
				...
				{
					Bkimg[0].Render(420 + x, 200, 0, 1, 1);	//회색블럭이미지
					...
					m_block[i][j].dx = m_blockW + x;
					m_block[i][j].dy = m_blockH;
				...
				else if (bkarr1[i][j] == 2)
				{
					...
					dv_font.Fonts->DrawTextA(NULL, "충", -1, &m_bkrc, DT_LEFT, D3DCOLOR_ARGB(255, 255, 0, 0));
					m_block[i][j].dx = m_blockW + y;
					m_block[i][j].dy = m_blockH + 20;
				...
				else if (bkarr1[i][j] == 3)
				{
					...
					dv_font.Fonts->DrawTextA(NULL, "충", -1, &m_bkrc, DT_LEFT, D3DCOLOR_ARGB(255, 255, 0, 0));
					m_block[i][j].dx = m_blockW + z;
					m_block[i][j].dy = m_blockH + 40;
				...
				else if (bkarr1[i][j] == 4)
				{
					...
					dv_font.Fonts->DrawTextA(NULL, "충", -1, &m_bkrc, DT_LEFT, D3DCOLOR_ARGB(255, 255, 0, 0));
					m_block[i][j].dx = m_blockW + a;
					m_block[i][j].dy = m_blockH + 60;
				...
				else if (bkarr1[i][j] == 5)
				{
					...
					dv_font.Fonts->DrawTextA(NULL, "충", -1, &m_bkrc, DT_LEFT, D3DCOLOR_ARGB(255, 255, 0, 0));
					m_block[i][j].dx = m_blockW + b;
					m_block[i][j].dy = m_blockH + 80;
...
void Blocks::Boom()
{
	...
	for (int i = 0; i < 5; i++)
	{
		for (int j = 0; j < 13; j++)
		{
			if (m_block[i][j].dx < ball.m_W + ball.m_HSize &&
				ball.m_W < m_block[i][j].dx + b_WSize &&
				m_block[i][j].dy < ball.m_H + ball.m_HSize &&
				ball.m_H < m_block[i][j].dy + b_HSize)
			{
				//공 튕기기
				float x, y;
